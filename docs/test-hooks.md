...
| `?game=tenbin\|ofune\|oyama` | タイトル/メニューを飛ばして直接入場。未知の値（旧 `wakekko` 含む）は tenbin にフォールバック。実経路（タイトル→メニュー→ゲーム）はナビゲーションシナリオが実タップで別途通す |
...
window.__tenbin = {
  ...
  game,             // 'tenbin' | 'ofune' | 'oyama' | null（screen==='game' 以外は null）
  menuTiles,        // screen==='menu' 時のみ {tenbin, ofune, oyama} 各{x,y,w,h}（論理座標）。他は null
  ...
  round,            // 進行中ラウンドのライブビュー（操作の瞬間に確定した値）。ゲーム外は null
                    ...
                    //   おやま:   {index, fruitsTotal, twins, towersAtClear, maxHeight,
                    //              slideOffs, takeBacks, dots}（v0.8。twins=同サイズ組の数）
  ...
  apples,           // [{id, x, y, r, state, plate, weight}] 論理座標。weight: 通常1/おおもの2(v0.7)。
                    ...
                    //   おやま   state: 'field'|'drag'|'tower'（動物なし＝eaten は無い）
  ...
  boat,             // おふね: {x, y, w, h, state:'dock'|'sailing'|'unloading'|'returning'|'held',
                    ...
                    //   てんびん/ゲーム外: null
  towers,           // おやま: [{x, y, sway, fruits:[id 下から順]}]（v0.8。sway=揺れ角・度）。
...
- ログ検証: `localStorage.getItem('tenbin_logs')` / `ofune_logs` / `oyama_logs` / `wakekko_logs`（legacy）を直接読む
