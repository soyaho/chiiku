...
| パラメータ | 意味 |
|---|---|
| `?seed=<uint>` | 乱数シード（mulberry32等）。配置・左右反転・テーマを決定的にする。省略時は非決定 |
| `?timescale=<float>` | シミュレーション時間の倍率（既定1）。固定タイムステップ（1/120s）の積分回数を増やして加速する。**物理のdtは変えない**（安定性維持）。ゲーム内の全タイマー（静止判定1s・遷移1.5s・長押し2s・おうち押し込み0.8s・演出シーケンス・ログのms計測）はシミュレーション時間基準にする |
| `?game=tenbin\|wakekko` | タイトル/メニューを飛ばして直接そのゲームへ入場（v0.3）。実経路（タイトル→メニュー→ゲーム）はナビゲーションシナリオが実タップで別途通す |
...
window.__tenbin = {
  ...
  game,             // 'tenbin' | 'wakekko' | null（screen==='game' 以外は null）
  menuTiles,        // screen==='menu' 時のみ {tenbin:{x,y,w,h}, wakekko:{x,y,w,h}}（論理座標）。他は null
  homeButton,       // screen==='game' 時のみ {x,y,w,h}（おうちボタンの押し込み領域）。他は null

  // --- ゲーム共通（わけっこ でも同じ意味。balance のみ てんびん専用） ---
  phase,            // 'play' | 'celebrate' | 'transition' | 'sessionEnd'
  problemIndex,     // 0..4
  levels,           // 現在のゲームのレベルテーブル（そのまま）
  apples,           // [{id, x, y, r, state, plate}] 論理座標。
                    //   state: 'field'|'drag'|'plate'|'ground'|'eaten'
                    //   plate: 'L'|'R'|null
  plates,           // {L:{x,y}, R:{x,y}} 皿中心の現在論理座標
  balance,          // てんびん: {angle, vel, target}（度。angle>0 = 右が下）。わけっこ: null
  wants,            // わけっこ: {left, right} 欲しい数。てんびん: null
  ...
  celebrationType,  // てんびん: 'left'|'right'|'balance'|null。わけっこ: 'both'|null
...
- 文字禁止の検証: ハーネスが `CanvasRenderingContext2D.prototype.fillText/strokeText` を
  ...
  （タイトル・メニュー・両ゲームすべてに適用）
- ログ検証: `localStorage.getItem('tenbin_logs')` / `wakekko_logs` を直接読む（読み取りのみ）
